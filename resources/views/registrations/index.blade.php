...
@if ($totalCount > 0)
    <div class="siperlo-surface rounded-md p-4 text-sm text-ink/80">
        Kamu mengikuti <span class="font-semibold text-ink">{{ $totalCount }} lomba</span>.
        <span class="font-semibold text-ink">{{ $monitoringCount }}</span> dalam monitoring,
        ...
        <article class="siperlo-surface divide-y divide-border-line rounded-md">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4 p-5">
                <div class="flex items-start gap-4">
                    <div class="flex h-16 w-12 shrink-0 overflow-hidden rounded-md border border-border-line bg-admin-note-surface">
                        <img src="{{ $posterUrl }}"
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="mt-2 text-sm text-ink/80">Status utama: {{ $registration->primaryStatusLabel() }}</div>
                    </div>
                </div>
            ...
            <div class="grid md:grid-cols-3">
                @foreach (\App\Models\Registration::PROGRESS_STEPS as $step)
                    <div class="flex items-center gap-3 border-b border-border-line px-5 py-3 text-sm font-semibold last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0 {{ $loop->iteration <= $statusRank ? 'bg-soft-green text-campus-green' : 'text-muted-ink' }}">
                        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold {{ $loop->iteration <= $statusRank ? 'bg-campus-green text-white' : 'border border-border-line bg-panel text-muted-ink' }}">{{ $loop->iteration }}</span>
                        <span>{{ $step }}</span>
                    </div>
                @endforeach
            </div>
            ...
            <div class="p-5">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-sm font-semibold text-ink">{{ $nextAction['title'] }}</div>
            ...
            {{-- Support region (mentor / dana / laporan) sebagai baris rapih tanpa sub-card --}}
            @php
                $supportRows = [];
