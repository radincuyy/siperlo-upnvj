...
@section('title', 'Review Mentor - SIPERLO')
@section('eyebrow', 'Admin')
@section('page_title', 'Review Pengajuan Mentor')
...
<section class="siperlo-surface rounded-md p-5">
    <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
            <h2 class="font-display text-xl font-bold">Daftar Pengajuan Mentor</h2>
            <p class="mt-1 text-sm text-ink/80">Prioritaskan pengajuan yang masih menunggu review.</p>
        </div>

        <form method="GET" action="{{ route('admin.mentor-requests.index') }}" class="grid gap-3 sm:grid-cols-[280px_auto]">
            <input type="hidden" name="status" value="{{ $selectedStatus }}">
            <div>
                <label for="mentor-search" class="text-sm font-semibold text-ink">Cari pengajuan</label>
                <input id="mentor-search" name="search" value="{{ $search }}" placeholder="Mahasiswa, mentor, atau lomba" class="siperlo-field mt-1 w-full">
            </div>
            <button class="siperlo-btn-primary self-end px-4 py-2 text-sm">Cari</button>
        </form>
    </div>

    <div class="mt-5 flex flex-wrap gap-2" aria-label="Filter status pengajuan mentor">
        @foreach ($statusTabs as $status => $label)
            @php
                $count = $status === 'all' ? $totalRequests : ($statusCounts[$status] ?? 0);
                $isActive = $selectedStatus === $status;
            @endphp
            <a href="{{ route('admin.mentor-requests.index', array_filter(['status' => $status, 'search' => $search ?: null])) }}"
...
<div class="mt-5 grid gap-4">
    @forelse ($requests as $request)
        ...
        <article class="siperlo-surface rounded-md p-5">
            <div class="grid gap-5 xl:grid-cols-[1fr_360px]">
                <div class="min-w-0">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <h2 class="mt-1 font-display text-xl font-bold leading-snug">
                                {{ $request->user->name }} mengajukan {{ $request->mentor->user->name }}
                            </h2>
                        </div>
                    ...
                    <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-3">
                        ...
                        <div class="siperlo-data-cell">
                            ...
                            <dd class="mt-1 text-ink/80">{{ $request->mentor->user->name }}</dd>
                        </div>
                        <div class="siperlo-data-cell">
                    ...
                    <div class="mt-4 rounded-md border border-border-line bg-panel p-4 text-sm leading-6 text-ink/80">
                        <div class="font-semibold text-ink">Alasan mahasiswa</div>
                ...
                @if ($isPending)
                    <form method="POST" action="{{ route('admin.mentor-requests.update', $request) }}" class="rounded-md border border-border-line bg-admin-note-surface p-4">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label for="mentor-status-{{ $request->id }}" class="text-sm font-semibold text-ink">Status review</label>
                            <select id="mentor-status-{{ $request->id }}" name="status" class="siperlo-field mt-1 w-full">
                                @foreach (\App\Models\MentorRequest::REVIEW_STATUSES as $status => $label)
                        ...
                        <div class="mt-3">
                            <label for="mentor-notes-{{ $request->id }}" class="text-sm font-semibold text-ink">Catatan admin</label>
                            <textarea id="mentor-notes-{{ $request->id }}" name="admin_notes" rows="4" placeholder="Tulis alasan keputusan" class="siperlo-field mt-1 w-full">{{ $request->admin_notes }}</textarea>
                        </div>
    ...
    @empty
        <div class="siperlo-empty">Tidak ada pengajuan mentor untuk filter ini.</div>
    @endforelse
</div>
